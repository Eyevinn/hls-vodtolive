...
class HLSVod {
  ...
  fromJSON(serialized) {
    ...
    this.targetProfiles = de.targetProfiles;
  }
      ...
      parser.on("m3u", (m3u) => {
        ...
        for (let i = 0; i < m3u.items.StreamItem.length; i++) {
          ...
          if (streamItem.get("bandwidth")) {
            let bw = streamItem.get("bandwidth");
            let usageProfile;
            let selectedBw = this.targetProfiles.length > 0 ? this._getNearestBandwidthInList(bw, this.targetProfiles) : bw;
            if (this.targetProfiles.length > 0 && (!this.usageProfile.length || !this.usageProfile.map((up) => up.bw).includes(selectedBw))) {
              usageProfile = {
                bw: bw,
              };
            } else if (this.targetProfiles.length > 0) {
              continue;
            } else {
              usageProfile = {
                bw: bw,
